...
export const getCart = async (req, res) => {
    try {
        ...
        if (!cart || cart.items.length === 0) {
            if (req.headers.accept && req.headers.accept.includes("application/json")) {
                // Если AJAX-запрос → отправляем JSON
                return res.json({ success: true, items: [], totalPrice: 0 });
            } else {
                // Если обычный запрос (браузер) → рендерим пустую корзину
                return res.render("cart", { items: [], totalPrice: 0, empty: true });
            }
        }

        // Если корзина не пустая, рендерим страницу или отправляем JSON
        if (req.headers.accept && req.headers.accept.includes("application/json")) {
    ...
    export const deleteFromCart = async (req, res) => {
        try {
            ...
            const { itemId } = req.body; // Получаем только itemId (теперь это _id элемента корзины)
            ...
            // Фильтруем элементы, оставляя только те, чей _id не совпадает с удаляемым
